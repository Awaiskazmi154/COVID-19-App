...
const WorldStatistics = (props) => {
  ...
  return (
    <View style={styles.container}>
      <View>
        <Text style={styles.result}>Total Population : {tpopulation}</Text>
      </View>
      <View>
      ...
      <View style={styles.result}>
        <Text style={styles.result}>
          Last Updated : {stats ? stats[0].lastUpdate : ''}
        </Text>
      </View>
...
const Countries = (props) => {
  ...
  useEffect(() => {
    ...
    const saveToFav = async (country, code) => {
      await AsyncStorage.setItem(country, code);
      console.log(country + ' stored');
      if (!getList.includes(country)) setList([...getList, country]);
    };
    saveToFav(country, code);
  }, [getdata]);

  useEffect(() => {
    const load = async () => {
      setList(await AsyncStorage.getAllKeys());
    };
    load();
  }, [getList]);
        ...
        {Object.entries(cnames.body.countries).map(([code, country]) => (
          <View style={styles.row}>
            <Text
              style={styles.result}
              onPress={() =>
                props.navigation.navigate('CountriesDetailsScreen', { country })
              }>
              {country}
            </Text>
            <CustomButton
              ...
              textSize={12}
              textColor={'white'}
              onPressEvent={() => setdata({ country, code })}
              pressed={getList.includes(country) ? true : false}
            />
          </View>
        ))}
      </ScrollView>
...
const FavCountries = (props) => {
  ...
  useEffect(() => {
    const del = async (getdata) => {
      await AsyncStorage.removeItem(getdata);
      console.log(getdata + ' deleted');
      setList(() => getList.filter((item) => item != getdata));
    };
    del(getdata);
  }, [getdata]);
      ...
      <ScrollView>
        {getList.map((country) => (
          <View style={styles.row}>
            ...
            <Button title="X" onPress={() => setdata(country)} />
          </View>
        ))}
      </ScrollView>
    </View>
